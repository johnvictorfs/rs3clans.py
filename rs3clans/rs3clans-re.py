...
class Clan:
    ...
    def __init__(self, name, set_exp=False, set_dict=True, rank_key='rank', exp_key='exp'):
        ...
        For self.member to be made, argument set_dict has to be True. (True by default)
        """
        self.name = name
        self.exp = None
        self.member = None
    ...
    def list_lookup(self):
        ...
        """
        url = f'http://services.runescape.com/m=clan-hiscores/members_lite.ws?clanName={self.name}'
        try:
...
class Player:
    ...
    def raw_info(self):
        url = (f"http://services.runescape.com/m=website-data/playerDetails.ws?names=%5B%22{self.name},"
